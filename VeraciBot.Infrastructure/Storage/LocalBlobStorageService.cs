...
namespace VeraciBot.Infrastructure.Storage
{
    public class LocalBlobStorageService : IBlobStorageService
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(objectName))
                throw new ArgumentException("O nome do arquivo nao pode ser vazio.", nameof(objectName));
            ...
            if (segments.Length == 0 || segments.Any(x => x == "." || x == ".."))
                throw new ArgumentException("Nome de arquivo invalido.", nameof(objectName));
