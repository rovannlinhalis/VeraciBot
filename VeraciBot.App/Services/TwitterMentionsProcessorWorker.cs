...
namespace VeraciBot.App.Services
{
    public sealed class TwitterMentionsProcessorWorker : BackgroundService
    {
        ...
        private static string ExtractFinalResponseText(
            ...
            string mentionText)
        {
            var structuredText = response?.Text?.Trim();
            if (!string.IsNullOrWhiteSpace(structuredText)
                && !LooksLikeContextEcho(structuredText, mentionText))
            {
        ...
        {
            var content = NormalizeSearchText(
        ...
        private static string BuildNewsSearchQuery(string mentionText, TwitterAPI.ThreadContext fullThread, string botUserId)
        {
            ...
            var threadCandidates = fullThread?.Tweets
            ...
            if (string.IsNullOrWhiteSpace(query) && fullThread != null)
            {
                query = SelectBestNewsSearchCandidate(new[]
                {
                    CleanNewsSearchCandidate(fullThread.GetStartA()),
                    CleanNewsSearchCandidate(fullThread.GetStartB())
                });
            }
        ...
        private static string CleanNewsSearchCandidate(string value)
        {
            ...
            var cleaned = TwitterAPI.RemoveReferences(value);
        ...
        private static string SelectBestNewsSearchCandidate(IReadOnlyCollection<string> candidates)
        {
            return candidates
                .Where(candidate => !string.IsNullOrWhiteSpace(candidate))
                .OrderBy(candidate => LooksLikeOnlyFactCheckRequest(candidate))
        ...
        private static TwitterAgentResponse TryParseAgentResponse(string text)
        {
            ...
            try
            {
                ...
                {
                    Result = ReadStringProperty(doc.RootElement, "result"),
                    Text = ReadStringProperty(doc.RootElement, "text")
                };
            }
            catch (JsonException)
            {
