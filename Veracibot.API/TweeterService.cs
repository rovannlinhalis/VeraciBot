...
namespace Veracibot.API
{
    public class TweeterService
    {
        ...
        /// </summary>
        /// <param name="id"></param>
        public static async Task<TweetAuthor> GetTweetAuthor(VeracibotDbContext dbContext, string id, string username = "", string name = "")
        {
        ...
        public static async Task<TwitterUser> GetTwitterUserById(string userId, HttpClient client, TweeterOptions options)
        {
        ...
        public static async Task<string> GetUsernameById(string id, HttpClient client, TweeterOptions options)
        {
        ...
        public static async Task<string> GetNameById(string id, HttpClient client, TweeterOptions options)
        {
            client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", options.BearerToken);
        ...
        /// <returns></returns>
        public static async Task<ThreadContext> GetThreadContext(string tweetId, string authorId, HttpClient httpClient, TweeterOptions options)
        {

            TweetContext tw = await GetTweetContext(tweetId, httpClient, options );
            if (tw == null)
            {
            ...
            else
            {
                ...
                ThreadContext tc = await GetThreadContext(tw.RepliedToId, authorId, httpClient, options);
                if (tc == null)
                {
        ...
        /// <param name="tweetId"></param>
        /// <returns></returns>
        public static async Task<TweetContext> GetTweetContext( string tweetId, HttpClient httpClient, TweeterOptions options)
        {
        ...
        public static async Task<string> GetRepliedTweetText(string tweetId, HttpClient httpClient, TweeterOptions options)
        {
            httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", options.BearerToken);
            ...
            if (root.TryGetProperty("referenced_tweets", out JsonElement referencedTweets))
            {
                foreach (var refTweet in referencedTweets.EnumerateArray())
                {
                    if (refTweet.GetProperty("type").GetString() == "replied_to")
                    {
                        ...
                        return await GetTweetTextById(repliedToId, httpClient, options);
                    }
                }
            }
        ...
        public static async Task<string> GetTweetTextById(string tweetId, HttpClient httpClient, TweeterOptions options)
        {
            httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", options.BearerToken);
        ...
        public static async Task PostReplyAsync(string message, string replyToTweetId, HttpClient client, TweeterOptions options)
        {
            ...
            try
            {
                ...
                string authHeader = GenerateOAuthHeader("POST", url, options);
        ...
        public static async Task PostReplyWithImageAsync(string message, string image, string replyToTweetId, HttpClient client, TweeterOptions options)
        {

            try
            {

                string mediaId = await UploadImageTweetinviAsync(image, client, options );
                if (mediaId == null)
                {
                ...
                var authHeader = GenerateOAuthHeader("POST", url, options);
        ...
        public static async Task<string> UploadImageTweetinviAsync(string image, HttpClient httpClient, TweeterOptions options)
        {

            var client = new TwitterClient(options.ApiKey, options.ApiSecret, options.AccessToken, options.AccessSecret);
        ...
        public static string GenerateOAuthHeader(string httpMethod, string url, TweeterOptions options, Dictionary<string, string> additionalParams = null, bool isUpload = false)
        {
            ...
            string signingKey = $"{Uri.EscapeDataString(options.ApiSecret)}&{Uri.EscapeDataString(options.AccessSecret)}";
